...
def get_credentials():
    ...
    if config.has_option('main', 'login'):
        ...
        user = input("Please, input your github login: ")
        ...
        if r.status_code != 200:
            print
            "Sorry, can't find user" + str(user)
             my_flag = False
        else:
        ...
        if len(argv) == 1:
            +    print
            "Usage: " + argv[0] + " username repository [options]"
        ...
        +        print
        "USERNAME: " + argv[1] + " REPOSITORY: " + argv[2]
        ...
        if v == 'all':
            +                    print
            "Total PR=" + str(all_pr) + ". Merged - " + str(merged) + ", closed - " + str(closed_pr)
        +
        if v == 'merged':
            +                    print
            "merged - " + str(merged)
        ...
        if int(comments) == 0:
            +                    print
            "Sorry, this PR seems to be closed."
        + else:
        +                    print
        "Total comments for PR #" + str(v) + " - " + str(comments)
        + elif s == '-l':
        ...
        if label:
            +                    print
            "Labels for PR #" + str(v) + ": " + str(label)
        + else:
        +                    print
        "No labels found"
        + elif s == '-u':
        +                print
        user_in_pr(main_link, str(v))
        + elif s == '-d':
        +                print
        "here!"
        +                main_link += '/pulls/' + str(v)
        +                print
        parse_date_param(main_link)
        + else:
        +    print_help()